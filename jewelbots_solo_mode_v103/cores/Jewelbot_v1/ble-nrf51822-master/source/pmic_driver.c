...
void pmic_init() {
  ...
  if (!success) {
    NRF_LOG_DEBUG("Error writing CHGCONFIG0 configuration!\r\n");
    return;
  }
...
// 0x4 = default current termination
  ...
  if (!success) {
    NRF_LOG_DEBUG("Error writing CHGCONFIG1 configuration!\r\n");
    return;
  }
  ...
  if (!success) {
    NRF_LOG_DEBUG("Error writing CHGCONFIG2 configuration!\r\n");
    return;
  }
  ...
  if (!success) {
    NRF_LOG_DEBUG("Error writing CHGCONFIG3 configuration!\r\n");
    return;
  }
...
// add voltage mask to explicitly set register to correct, default values
  ...
  if (!success) {
    NRF_LOG_DEBUG("Error writing DCDC1 configuration!\r\n");
    return;
  }
...
/*Configure LDO_CTRL register */
  ...
  if (!success) {
    NRF_LOG_DEBUG("Error writing PMIC_REG_IRMASK0 configuration!\r\n");
    return;
  }
...
void pmic_disable() {
  ...
  success = pmic_driver_read_reg(PMIC_REG_DEFDCDC1, &val);
  if (!success) {
    NRF_LOG_DEBUG("Error reading DCDC1 configuration!\r\n");
    return;
  }
...
bool pmic_is_charging(void) {
  ...
  if (!success) {
    NRF_LOG_DEBUG("Error reading CHGSTATUS register!\r\n");
    return false;
  }
...
bool pmic_toggle_charging() {
  ...
  NRF_LOG_DEBUG("Toggle Charging\n");
  success = pmic_driver_read_reg(PMIC_REG_CHGCONFIG0, &val);
  NRF_LOG_PRINTF_DEBUG("CHGCONFIG0 val after read is: %02x\r\n", val);
  if (!success) {
    NRF_LOG_DEBUG("failed to read CHGCONFIG0 register!\r\n");
    return false;
  }
  val = val ^ (1 << 0);
  NRF_LOG_PRINTF_DEBUG("CHGCONFIG0 val after change is: %02x\r\n", val);
  success = pmic_driver_write_reg(PMIC_REG_CHGCONFIG0, val);
  if (!success) {
    NRF_LOG_DEBUG("failed to write CHGCONFIG0 register!\r\n");
    return false;
  }
  return true;
...
bool pmic_enable_dynamic_ppm(void) {
  ...
  uint8_t success;
  uint8_t val = 0;
  NRF_LOG_DEBUG("Turn on Dynamic PPM\n");
  success = pmic_driver_read_reg(PMIC_REG_CHGCONFIG0, &val);
  NRF_LOG_PRINTF_DEBUG("CHGCONFIG0 val after read is: %02x\r\n", val);
  if (!success) {
    NRF_LOG_DEBUG("failed to read CHGCONFIG0 register!\r\n");
...
bool pmic_disable_dynamic_ppm(void) {
  ...
  uint8_t success;
  uint8_t val = 0;
  NRF_LOG_DEBUG("Turn off Dynamic PPM\n");
  success = pmic_driver_read_reg(PMIC_REG_CHGCONFIG0, &val);
  NRF_LOG_PRINTF_DEBUG("CHGCONFIG0 val after read is: %02x\r\n", val);
  if (!success) {
    NRF_LOG_DEBUG("failed to read CHGCONFIG0 register!\r\n");
